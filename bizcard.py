...
import re 
import io
import requests                                       #text processing
import pandas as pd
import numpy as np
import time 
...
icon = Image.open("D:\\Creative Modern Business Card\\business (1).png")
st.set_page_config(page_title="BizCardX: Extracting Business Card Data with OCR | By VIKRAM HUGGI",
                   page_icon=icon,
                   layout="wide",
                   initial_sidebar_state="expanded",
                   menu_items={'About': """# This OCR app is created by *VIKRAM HUGGI*!"""})
...
if selected == "Home":
    ...
    with col1:
        st.image(Image.open("D:\\Creative Modern Business Card\\business (1).png"), width=500)
        #st.markdown("## :blue[**Technologies Used :**] Python,easy OCR, Streamlit, SQL, Pandas")
        st.markdown("## <span style='color: blue;'>Technologies Used:</span> Python, easy OCR, Streamlit, SQL, Pandas", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
    ...
    <div style='color: blue;'>
        <h2>The main purpose of Bizcard is to automate the process of extracting key details from business card images, such as the name, designation, company, contact information, and other relevant data. By leveraging the power of OCR (Optical Character Recognition) provided by EasyOCR, Bizcard is able to extract text from the images.</h2>
    </div>
""", unsafe_allow_html=True)
...
if selected == "Delete":
    ...
    with col1:
        mycursor.execute("SELECT NAME FROM biz_card_data")
        Y = mycursor.fetchall()
        names = ["Select"]
        for i in Y:
            names.append(i[0])
    ...
    with col2:
        mycursor.execute(f"SELECT DESIGNATION FROM biz_card_data WHERE NAME = '{name_selected}'")
        Z = mycursor.fetchall()
        designation = ["Select"]
        for j in Z:
    ...
    if name_selected and designation_selected and remove:
        mycursor.execute(
            f"DELETE FROM biz_card_data WHERE NAME = '{name_selected}' AND DESIGNATION = '{designation_selected}'")
        mydb.commit()
        if remove:
...
result = ["John Doe", "Manager", "+123456789", "john@example.com", "www.example.com", "12345", "Company XYZ", "123 Main St"]
